...
--
-- Hôte : db
-- Généré le :  mar. 01 mai 2018 à 18:51
...
-- Procédures
--
CREATE DEFINER=`root`@`%` PROCEDURE `meilleurs_acheteurs` ()  BEGIN
    SELECT u.*, mu.nombre_achats
    FROM utilisateur AS u
    LEFT JOIN (
        SELECT id_utilisateur,
        SUM(quantite) AS nombre_achats
        FROM commande
        WHERE type='achat'
        GROUP BY id_utilisateur
    ) AS mu
    ON u.id_utilisateur = mu.id_utilisateur
    ORDER BY mu.nombre_achats DESC
    LIMIT 5;
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `meilleurs_ventes` ()  NO SQL
BEGIN
    SELECT o.*, mv.nombre_ventes
    FROM objet AS o
    LEFT JOIN (
        SELECT
        id_objet, COUNT(id_objet) as nombre_ventes
        FROM acheter
        GROUP BY id_objet
    ) AS mv
    ON o.id_objet = mv.id_objet
    ORDER BY mv.nombre_ventes DESC
    LIMIT 5;
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `meilleur_payeur` ()  NO SQL
BEGIN
    SELECT u.*, mu.total_prix_achat
    FROM utilisateur AS u
    INNER JOIN (
        SELECT id_utilisateur,
        SUM(prix_total) AS total_prix_achat
        FROM commande
        WHERE type='achat'
        GROUP BY id_utilisateur
        LIMIT 1
    ...
    ON u.id_utilisateur = mu.id_utilisateur
    ORDER BY mu.total_prix_achat DESC;
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `pires_acheteurs` ()  BEGIN
    SELECT u.*, mu.nombre_achats
    FROM utilisateur AS u
    LEFT JOIN (
        SELECT id_utilisateur,
    ...
    ON u.id_utilisateur = mu.id_utilisateur
    ORDER BY mu.nombre_achats ASC
    LIMIT 5;
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `pires_ventes` ()  NO SQL
BEGIN
    SELECT o.*, mv.nombre_ventes
    FROM objet AS o
    LEFT JOIN (
...
INSERT INTO `commande` (`id_commande`, `date_obtention`, `quantite`, `prix_total`, `type`, `id_utilisateur`) VALUES
(5, '2017-12-19-14:49:55', '1', 1314.99, 'achat', 2),
(6, '2017-12-19-14:51:12', '1', 1769.99, 'achat', 2),
(7, '2017-12-19-16:41:09', '1', 2224.99, 'achat', 2),
(9, '2017-12-21-14:19:17', '1', 1514.99, 'achat', 2),
(11, '2018-01-11-12:05:00', '1', 1182, 'achat', 2),
(12, '2018-01-11-12:05:18', '1', 1182, 'achat', 2),
(13, '2018-01-11-12:06:09', '1', 1182, 'achat', 2),
(14, '2018-01-11-13:20:43', '1', 1514.99, 'achat', 2),
(15, '2018-03-14-10:01:57', '1', 470, 'achat', 5);
